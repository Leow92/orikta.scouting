...
def _similar_players_core(
    ...
    # ---- 2. Single reference pool for ALL percentile computations ----
    ...
    # ---- 3. Compute similarity for each pool player ----
    candidates = []
    for p_obj, p_metrics in pool_entries:
        ...
        candidates.append({
            "name": p_name, "team": p_team, "age": p_age,
            "position": p_pos, "similarity": sim, "pcts": p_pct,
        })
    ...
    photo_html = (
        ...
        + "</div>"
    ) if (photo_url and show_photo) else ""
...
def similar_players(
    ...
    try:
        ...
        return _similar_players_core(player_obj, pool, scout_df, language=language, show_photo=show_photo)
